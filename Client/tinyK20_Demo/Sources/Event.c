 ...
 *      Author: daniel
 *
 *      implements a generic event driver. the events are numbered starting with zero.
 *      the Method EVNT_HandleEvent() can be used to process the pending events. Note that the event with the number zero has the highest priority
 *      and will be handled first.
 */

#include "Platform.h"
#if PL_CONFIG_HAS_EVENTS
#include "Event.h"
#if PL_CONFIG_HAS_CRITICAL
#include "CS1.h"
#endif
